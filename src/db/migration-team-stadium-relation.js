...
try {
  ...
  const [teams] = await connection.execute('SELECT id, name, home_stadium FROM teams');
  ...
  for (const team of teams) {
    if (team.home_stadium) {
      ...
      if (stadiumRows.length > 0) {
        ...
        console.log(`✅ ${team.name}: ${team.home_stadium} → stadium_id: ${stadiumId}`);
      } else {
        console.log(`⚠️ ${team.name}: ${team.home_stadium}에 해당하는 stadium을 찾을 수 없음`);
      }
    }
  }
