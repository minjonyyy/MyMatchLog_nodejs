...
export const createEvent = async (eventData) => {
  ...
  if (!title || !description || !start_at || !end_at || !gift || !capacity) {
    throw new BadRequestError('모든 필수 필드를 입력해주세요.', 'ADMIN_MISSING_REQUIRED_FIELDS');
  }
  ...
  if (startDate <= now) {
    throw new BadRequestError('이벤트 시작 시간은 현재 시간보다 이후여야 합니다.', 'ADMIN_INVALID_START_TIME');
  }
  
  if (endDate <= startDate) {
    throw new BadRequestError('이벤트 종료 시간은 시작 시간보다 이후여야 합니다.', 'ADMIN_INVALID_END_TIME');
  }
  
  // 정원 검증
  if (capacity <= 0) {
    throw new BadRequestError('정원은 1명 이상이어야 합니다.', 'ADMIN_INVALID_CAPACITY');
  }
...
export const getEventParticipants = async (eventId) => {
  ...
  if (!event) {
    throw new NotFoundError('해당 이벤트를 찾을 수 없습니다.', 'ADMIN_EVENT_NOT_FOUND');
  }
