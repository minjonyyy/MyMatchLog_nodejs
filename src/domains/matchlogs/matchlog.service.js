import { NotFoundError, ForbiddenError, BadRequestError } from '../../errors/http.error.js';
...
const validateTeamExists = async (teamId) => {
  ...
  if (!teamExists) {
    throw new BadRequestError(`팀 ID ${teamId}는 존재하지 않습니다.`, 'COMMON_INVALID_INPUT');
  }
};
...
const validateStadiumExists = async (stadiumId) => {
  ...
  if (!stadium) {
    throw new BadRequestError(`경기장 ID ${stadiumId}는 존재하지 않습니다.`, 'COMMON_INVALID_INPUT');
  }
};
...
const validateMatchDate = (matchDate) => {
  ...
  if (inputDate > today) {
    throw new BadRequestError('시작 이전의 경기는 기록할 수 없습니다. 오늘까지의 경기만 기록 가능합니다.', 'INVALID_MATCH_DATE');
  }
};
...
export const getMatchLogDetail = async (matchLogId, userId) => {
  ...
  if (!matchLog) {
    throw new NotFoundError('해당 직관 기록을 찾을 수 없습니다.', 'MATCH_LOG_NOT_FOUND');
  }

  // 소유권 검증 - 본인의 기록만 조회 가능
  if (matchLog.user_id !== userId) {
    throw new ForbiddenError('해당 기록에 대한 접근 권한이 없습니다.', 'MATCH_INVALID_OWNER');
  }
...
export const createMatchLog = async (userId, matchLogData) => {
  const { match_date, home_team_id, away_team_id, stadium_id, result, memo, ticket_image_url } = matchLogData;

  // 필수 필드 검증
  if (!match_date || !home_team_id || !away_team_id || !stadium_id) {
    throw new BadRequestError('경기 날짜, 홈팀, 원정팀, 경기장은 필수 입력 항목입니다.', 'COMMON_INVALID_INPUT');
  }

  // 팀 ID 타입 검증
  if (!Number.isInteger(home_team_id) || !Number.isInteger(away_team_id)) {
    throw new BadRequestError('홈팀과 원정팀 ID는 정수여야 합니다.', 'COMMON_INVALID_INPUT');
  }

  // 경기장 ID 타입 검증
  if (!Number.isInteger(stadium_id)) {
    throw new BadRequestError('경기장 ID는 정수여야 합니다.', 'COMMON_INVALID_INPUT');
  }

  // 같은 팀끼리 경기 불가 검증
  if (home_team_id === away_team_id) {
    throw new BadRequestError('홈팀과 원정팀은 같을 수 없습니다.', 'COMMON_INVALID_INPUT');
  }
  ...
  if (result && !['WIN', 'LOSS', 'DRAW'].includes(result)) {
    throw new BadRequestError('경기 결과는 WIN, LOSS, DRAW 중 하나여야 합니다.', 'COMMON_INVALID_INPUT');
  }
...
export const updateMatchLog = async (matchLogId, userId, updateData) => {
  // 기존 기록 조회 및 소유권 검증
  const existingMatchLog = await matchLogRepository.findMatchLogById(matchLogId);
  ...
  if (existingMatchLog.user_id !== userId) {
    throw new ForbiddenError('해당 기록에 대한 수정 권한이 없습니다.', 'MATCH_INVALID_OWNER');
  }
  ...
  // 팀 ID 유효성 검증
  if (updateData.home_team_id !== undefined || updateData.away_team_id !== undefined) {
    ...
    // 팀 ID 타입 검증
    if (updateData.home_team_id !== undefined && !Number.isInteger(updateData.home_team_id)) {
      throw new BadRequestError('홈팀 ID는 정수여야 합니다.', 'COMMON_INVALID_INPUT');
    }
    if (updateData.away_team_id !== undefined && !Number.isInteger(updateData.away_team_id)) {
      throw new BadRequestError('원정팀 ID는 정수여야 합니다.', 'COMMON_INVALID_INPUT');
    }
  ...
  // 경기 결과 유효성 검증
  if (updateData.result && !['WIN', 'LOSS', 'DRAW'].includes(updateData.result)) {
    throw new BadRequestError('경기 결과는 WIN, LOSS, DRAW 중 하나여야 합니다.', 'COMMON_INVALID_INPUT');
  }
...
export const deleteMatchLog = async (matchLogId, userId) => {
  ...
  if (existingMatchLog.user_id !== userId) {
    throw new ForbiddenError('해당 기록에 대한 삭제 권한이 없습니다.', 'MATCH_INVALID_OWNER');
  }
