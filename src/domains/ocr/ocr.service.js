  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (line.includes('경기장:') || line.includes('구장:')) {
        const stadiumMatch =
          line.match(/경기장:\s*(.+)/) || line.match(/구장:\s*(.+)/);
        if (stadiumMatch) {
      ...
      if (line.includes('홈팀:') || line.includes('홈:')) {
        const homeTeamMatch =
          line.match(/홈팀:\s*(.+)/) || line.match(/홈:\s*(.+)/);
        if (homeTeamMatch) {
    ...
    // 최소한의 필수 정보가 있는지 확인
    if (
      parsedInfo.match_date &&
      parsedInfo.stadium &&
      parsedInfo.home_team &&
      parsedInfo.away_team
    ) {
      return parsedInfo;
    }
...
const validateAndMatchInfo = async (parsedInfo) => {
  try {
    ...
    const matchedStadium = stadiums.find(
      (stadium) =>
